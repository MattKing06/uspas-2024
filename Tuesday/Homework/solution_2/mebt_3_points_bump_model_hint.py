...
trMatrixNodes = trMatricesGenerator.addTrMatrxGenNodesAtEntrance(
    accLattice, lattice_trMatrix_nodes
)
...
for trMtrxNode in trMatrixNodes:
    print(
        ...
        trMtrxNode.getName(),
        " pos[m] = %5.3f" % trMtrxNode.getPosition(),
    )
...
bpm_ver_pos_values = [pv.get() for pv in bpm_ver_pos_pv_arr]
y_delta_vectors = []
for i in range(0, len(bpm_ver_pos_pv_arr) - 1):
    print(bpm_ver_pos_pv_arr[i].pvname)
    y_delta_vectors.append([0, bpm_ver_pos_values[i+1]-bpm_ver_pos_values[i]])
dcv_field_pv_arr[1].put(y_delta_vectors[1][1])
time.sleep(2.1)
for pv in bpm_ver_pos_pv_arr:
    yAvg = pv.get()
    print("debug bpm pv =", pv.pvname, " yAvg[mm]= %+6.4f" % (yAvg))
